...
* ingestfile is a combination of multiple microservices for the purposes of creating an Archival Information Package (AIP) from an input of a video file. Included in the AIP are the original file, access copies, and corresponding metadata. Running ingestfile also sets a unique identifier (UI) for a video file, and bases the directory and metadata naming on this UI. to run ingestfile, you must have the following variables set: OUTDIR_INGESTFILE, AIP_STORAGE, PODCASTDELIVER, YOUTUBEDELIVER. the default use of ingestfile runs the following processes on a file:  
    ...
    * ingestfile then sets up the structure of the AIP, writes out the input from the user queries to a log file, and uses rsync to move the original file into the objects directory.   
    * ingestfile transcodes a file for broadcast and delivers it to the omneon server.    
    * ingestfile creates access copies using makemp3, makeframes, makeyoutube, and makepodcast, and delivers youtube and podcast files.  
    * ingestfile creates metadata, checksums, and maps the directory's contents. 
    * ingestfile finishes by delivering the AIP to the specified AIP_STORAGE location. 
...
* if you want to run the video through a volume adjustment filter, then adjust the volume accordingly during transcoding, use option v. This process will be slower. Defaults to ${VOLADJUST} variable set in mmconfig. Your command will look like this: makebroadcast -v Y|N [input]. 
* makebroadcast has options d, o, n, e, E. To reread what those options mean, return to [link to section header]. If you use options o or d, your command will look like this: makebroadcast -o OR -d [path/to/directory] [input]. If you use option n, your command will look like this: makebroadcast -n [input]. If you use option e or E, your command will look like this: makebroadcast -e OR -E [input].  
...
#### makepdf ####
* makepdf creates a PDF file from a set of images. To use makepdf, you must have the following dependencies installed: 
